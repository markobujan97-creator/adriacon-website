...
export function TrustRow() {
  return (
    <section aria-label="Was Sie erwarten können" className="border-y border-line bg-shell">
      <div className="shell py-12">
        <ul className="grid gap-x-12 gap-y-8 sm:grid-cols-3">
          {trustPoints.map((point, i) => (
            <Reveal as="li" key={point.title} delay={i * 0.06}>
              <h2 className="font-display text-[1.1rem] font-medium text-navy">{point.title}</h2>
